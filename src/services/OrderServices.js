...
const OrderServices = {
  ...
  getDashboardRecentOrder: async ({ page = 1, limit = 8 }) => {
    return requests.get(`/orders/dashboard-recent-order?page=${page}&limit=${limit}`)
  },
  ...
  getTotalSoldByProduct: async (productId) => {
    return requests.get(`/orders/total-sold/${productId}`)
  },
}
