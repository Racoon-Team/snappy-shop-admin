...
const dynamicDataSlice = createSlice({
  ...
  reducers: {
    ...
    addSingleData: (state, action) => {
      ...
      const uniqueArray = uniqueArrayOfObjects([...(singleItem?.data || []), action.payload.data])
    ...
    updateMultipleData: (state, action) => {
      ...
      const updatedFinalArr = [...multipleUpdatedData, ...(action?.payload?.data || [])]

      const updatedDatMultiple = [...notUpdatedMultipleItems, { name: updatedMultipleItem.name, data: updatedFinalArr }]
