...
const CardItemTwo = ({ mode, title, Icon, className, price, cash, card, credit, loading, title2 }) => {
        ...
        <>
          {title === 'Today Order' || title === 'Yesterday Order' ? (
                ...
                <div className="text-center xl:mb-0 mb-3">
                  ...
                  <div className="flex text-center text-xs font-normal text-gray-50 dark:text-gray-100">
                    <div className="px-1 mt-3">
                      {t('dashboardScreen.cards.method.cash')} : {currency}
                      {getNumberTwo(cash)}
                    </div>
                    <div className="px-1 mt-3">
                      {t('dashboardScreen.cards.method.card')} : {currency}
                      {getNumberTwo(card)}
                    </div>
                    <div className="px-1 mt-3">
                      {t('dashboardScreen.cards.method.credit')} : {currency}
                      {getNumberTwo(credit)}
                    </div>
                  </div>
                </div>
            ...
            <Card className="flex justify-center text-center h-full">
              <CardBody className={`border border-gray-200 dark:border-gray-800 w-full p-6 rounded-lg ${className}`}>
                <div className={`text-center inline-block text-3xl ${className}`}>
                  <Icon />
                </div>
                <div>
                  <p className="mb-3 text-base font-medium text-gray-50 dark:text-gray-100">{t(`${title2}`)}</p>
                  <p className="text-2xl font-bold leading-none text-gray-50 dark:text-gray-50">
                    {/* ${Math.round(price)} */}
                    {currency}
                    {getNumberTwo(price)}
