...
const useProductSubmit = (id) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (data.originalPrice < data.price) {
        ...
        return notifyError(t('productsScreen.drawer.notification'))
      }
      if (!defaultCategory[0]) {
        setIsSubmitting(false)
        return notifyError(t('productsScreen.message.categoryNotification'))
      }
      ...
      const titleTranslates = await handlerTextTranslateHandler(data.title, language, resData?.title)
      const descriptionTranslates = await handlerTextTranslateHandler(data.description, language, resData?.description)
      ...
      if (updatedId) {
        ...
        if (res) {
          if (isCombination) {
            setIsUpdate(true)
            notifySuccess(t('productsScreen.message.updateError'))
            setIsBasicComplete(true)
            setIsSubmitting(false)
            handleProductTap('Combination', true)
          } else {
            setIsUpdate(true)
            notifySuccess(t('productsScreen.message.update'))
            setIsSubmitting(false)
          }
        }

        if (tapValue === 'Combination' || (tapValue !== 'Combination' && !isCombination)) {
          closeDrawer()
        }
      } else {
        ...
        if (isCombination) {
          ...
          const result = res.variants.map(
            ({ originalPrice, price, discount, quantity, barcode, sku, productId, image, ...rest }) => rest
          )
          ...
          setIsUpdate(true)
          notifySuccess(t('productsScreen.message.addNotification'))
        }
      ...
      ;(async () => {
        try {
          ...
          if (res) {
            ...
            res.category.name = showingTranslateValue(res?.category?.name, lang)
  ...
  }, [id, setValue, isDrawerOpen, location.pathname, clearErrors, language, lang])
  ...
  const handleGenerateCombination = () => {
    if (Object.keys(values).length === 0) {
      return notifyError(t('productsScreen.drawer.variantNotification'))
    }

    const result = variants.filter(
      ({ originalPrice, discount, price, quantity, barcode, sku, productId, image, ...rest }) =>
        JSON.stringify({ ...rest }) !== '{}'
    )
  ...
  const handleClearVariant = () => {
    ...
    setVariant([])
    setValues({})
    resetRef?.current?.map(async (v, i) => await resetRef?.current[i]?.resetSelectedValues())
  ...
  const handleRemoveVariant = (vari, ext) => {
    ...
    swal({
      title: t('productsScreen.productDetails.table.delete', { type: ext ? 'Extra' : 'combination' }),
      text: t('productsScreen.productDetails.table.deleteConfirmNote', { type: ext ? 'Extra' : 'combination' }),
      icon: 'warning',
      buttons: true,
      dangerMode: true,
    }).then((willDelete) => {
      if (willDelete) {
        ...
        setVariants(result)
        // console.log("result", result);
        const { originalPrice, price, discount, quantity, barcode, sku, productId, image, ...rest } = vari
        const res = variant.filter((obj) => JSON.stringify(obj) !== JSON.stringify(rest))
        setVariant(res)
  ...
  const handleIsCombination = () => {
    if ((isCombination && variantTitle.length) > 0) {
      swal({
        title: t('productsScreen.productDetails.table.question'),
        text: t('productsScreen.productDetails.table.description'),
        icon: 'warning',
        buttons: true,
        dangerMode: true,
      }).then((value) => {
  ...
  const handleProductTap = (e, value, name) => {
    ...
    if (value) {
      if (!value) return notifyError(`${'Please save product before adding combinations!'}`)
    } else {
      if (!isBasicComplete) return notifyError(`${t('productsScreen.message.saveNotification')}`)
    }
    setTapValue(e)
  }
  ...
  const handleQuantityPrice = (value, name, id, variant) => {
    ...
    const totalStock = variants.reduce((pre, acc) => Number(pre) + Number(acc.quantity), 0)
    setTotalStock(Number(totalStock))
  }
