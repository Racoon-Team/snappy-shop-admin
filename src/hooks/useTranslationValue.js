...
const useTranslationValue = () => {
  ...
  const handleTranslateCallApi = async (text, tnsForm, tnsTo) => {
    const key = globalSetting?.translation_key || import.meta.env.VITE_APP_MYMEMORY_API_KEY
    try {
  ...
  const handlerTextTranslateHandler = async (text, tnsForm, currentData) => {
    ...
    let objectTnsLanguage = languageArray.reduce((obj, item) => Object.assign(obj, { [item.lang]: item.text }), {})
