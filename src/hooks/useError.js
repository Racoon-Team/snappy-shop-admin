...
const useError = () => {
  ...
  const handleErrorNotification = async (err, from, time = 1000) => {
    console.log(`handleErrorNotification, error on ${from}`, err?.response?.data?.message || err?.message)
    if (
      ...
      const timeoutId = setTimeout(() => {
        ...
        window.location.replace(`https://${import.meta.env.VITE_APP_ADMIN_DOMAIN}/login`)
      }, 2500)
