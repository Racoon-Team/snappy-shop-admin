...
const useCategorySubmit = (id, data) => {
  ...
  const onSubmit = async ({ name, description }) => {
    try {
      ...
      // return;
      const descriptionTranslates = await handlerTextTranslateHandler(description, language, resData?.description)
      ...
      if (id) {
        ...
        setIsUpdate(true)
        setIsSubmitting(false)
        notifySuccess(t('categoriesScreen.message.updated'))
        closeDrawer()
        reset()
      } else {
        ...
        setIsUpdate(true)
        setIsSubmitting(false)
        notifySuccess(t('categoriesScreen.message.added'))
        closeDrawer()
      }
    } catch (err) {
      setIsSubmitting(false)
