...
const ChildCategory = () => {
  ...
  const { totalResults, resultsPerPage, dataTable, serviceData, handleChangePage } = useFilter(childCategory)
  ...
  return (
    <>
      <PageTitle>{t('categoriesScreen.title')}</PageTitle>

      <DeleteModal ids={allId} setIsCheck={setIsCheck} category />

      <BulkActionDrawer ids={allId} title="Child Categories" lang={lang} data={data} childId={id} />

      <AnimatedContent>
        <div className="flex items-center pb-4">
          <ol className="flex items-center w-full overflow-hidden font-serif">
            <li className="text-sm pr-1 transition duration-200 ease-in cursor-pointer hover:text-emerald-500 font-semibold">
              <Link to={`/categories`}>{t('categoriesScreen.linkCategories')}</Link>
            </li>
            {selectedObj?.map((child, i) => (
              <span key={i + 1} className="flex items-center font-serif">
                ...
                <li className="text-sm pl-1 transition duration-200 ease-in cursor-pointer text-blue-700 hover:text-emerald-500 font-semibold ">
                  <Link to={`/categories/${child._id}`}>{showingTranslateValue(child?.name)}</Link>
                </li>
              </span>
            ))}
          </ol>
          ...
          <CardBody>
            <div className="flex justify-end items-end">
              <Button onClick={toggleDrawer} className="rounded-md h-12">
                ...
                {t('categoriesScreen.addCategory')}
              </Button>
          ...
          <Table>
            <TableHeader>
              <tr>
                ...
                <TableCell>{t('categoriesScreen.table.catId')}</TableCell>
                <TableCell>{t('categoriesScreen.table.catIcon')}</TableCell>
                <TableCell>{t('categoriesScreen.table.catName')}</TableCell>
                <TableCell>{t('categoriesScreen.table.catDescription')}</TableCell>
                <TableCell className="text-center">{t('categoriesScreen.table.catPublished')}</TableCell>
                <TableCell className="text-right">{t('categoriesScreen.table.catActions')}</TableCell>
              </tr>
            </TableHeader>
